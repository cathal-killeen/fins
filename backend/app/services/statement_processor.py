    ...
    async def process_statement(
        ...
        try:
            ...
            status.update(
                ...
                10,
                f"Parsing {file_type.upper()} file..."
            )
            ...
            status.update(
                ProcessingStage.MATCHING_ACCOUNT,
                45,
                "Matching to your accounts..."
            )

            user_accounts = await self._get_user_accounts(user_id)
            account_match = await suggest_account_match(statement_metadata, user_accounts)
